...
export default {
  ...
  methods: {
    ...
    allowDrop (draggingNode, dropNode, type) {
      // 1、被拖动的当前节点一级所在的父节点总层数不能大于3
      // 1.1、被拖动的当前节点总层数
      console.log('allowDrop：', draggingNode, dropNode, type)
      this.countNodeLevel(draggingNode)
      // 当前正在拖动的节点+父节点所在的深度不大于3即可
      // let deep = this.maxLevel - draggingNode.data.catLevel + 1 以当前节点查看深度，不以数据库查看深度
    ...
    handleDrop (draggingNode, dropNode, dropType, ev) {
      ...
      if (dropType === 'before' || dropType === 'after') {
        ...
        siblings = dropNode.children.childNodes
      }
      // 全局pCid保存
      this.pCid.push(pCid)
    ...
    updateChildNodeLevel (node) {
      if (node.children.length > 0) {
        for (let i = 0; i < node.childNodes.length; i++) {
          var cNode = node.childNode[i].data
    ...
    edit (data) {
      ...
      this.$http({
        ...
        this.category.cateLevel = data.data.cateLevel
